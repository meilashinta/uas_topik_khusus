...
- [x] Buat `SubmitRatingCommand` handler:
  1. Cari tiket by ID
...
- [x] Rating sudah included di response `GET /api/v1/tickets/:id`
- [x] Pastikan hanya tampil jika sudah ada rating

### 5.4 Get Technician Ratings

- [x] Buat query untuk menampilkan statistik rating per technician:
  - Rata-rata score
  - Total rating
  - Distribusi score (berapa banyak score 1, 2, 3, 4, 5)
- [x] Endpoint ini akan digunakan di Dashboard (Phase 5)

---

## Definition of Done

- [x] Employee bisa submit rating (1-5) + feedback setelah tiket RESOLVED/CLOSED
- [x] 1 tiket hanya boleh 1 rating (unique constraint)
- [x] Statistik rating per technician bisa di-query
- [x] Swagger docs lengkap
- [x] Unit test: submit rating, duplicate prevention, score validation
