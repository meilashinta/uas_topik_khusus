...
- [x] Blacklist refresh token lama di Redis dengan TTL = sisa masa berlaku
- [x] Tandai dengan `@Public()`
...
- [x] Buat handler:
  ...
  3. Return sukses
- [x] Endpoint ini membutuhkan auth (access token valid)

### 2.3 Forgot Password — `POST /api/v1/auth/forgot-password`

- [x] Buat DTO `ForgotPasswordDto`:
  - `email` — string, wajib
- [x] Buat handler:
...
- [x] Tandai dengan `@Public()`
- [x] Buat template email reset password (HTML)

### 2.4 Reset Password — `POST /api/v1/auth/reset-password`

- [x] Buat DTO `ResetPasswordDto`:
  - `token` — string, wajib
...
- [x] Buat handler:
  ...
  5. Return sukses
- [x] Endpoint ini membutuhkan auth

### 2.6 Password History

- [x] Tambahkan model/field untuk menyimpan 3 password hash terakhir per user
...
- [x] Update migrasi Prisma jika menambah tabel/field baru
- [x] Saat change password:
...
- [x] Buat `EmailModule` dan `EmailService`
- [x] Konfigurasi SMTP menggunakan `nodemailer`:
  - Host, port, user, pass dari environment variable
  - Development: Mailtrap
- [x] Buat method `sendResetPasswordEmail(to, resetLink)`
- [x] Buat HTML template sederhana untuk email reset
- [x] Buat unit test mock untuk `EmailService`

---

## Definition of Done

- [x] Refresh token rotation bekerja (token lama di-blacklist, token baru digenerate)
- [x] Logout berhasil blacklist refresh token
- [x] Forgot password mengirim email ke Mailtrap
- [x] Reset password mengubah password dan token sekali pakai
- [x] Change password memvalidasi password lama & mengecek password history
- [x] Semua endpoint return format response standar
- [x] Swagger docs lengkap
- [x] Unit test: refresh token rotation, logout blacklist, password history check
