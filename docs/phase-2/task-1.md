...
- [x] Tambahkan Swagger decorator (`@ApiProperty`) di setiap field

### 1.2 Register Endpoint — `POST /api/v1/auth/register`

- [x] Buat `RegisterCommand` handler (CQRS pattern):
...
- [x] Tambahkan Swagger documentation
- [x] Tandai endpoint dengan `@Public()` (tidak butuh auth)

### 1.3 Login Endpoint — `POST /api/v1/auth/login`

- [x] Buat `LoginCommand` handler:
...
- [x] Tambahkan Swagger documentation
- [x] Tandai endpoint dengan `@Public()`

### 1.4 Rate Limiting Login (Redis)

- [x] Implementasi rate limiter di `AuthService`:
...
- [x] Buat unit test: verifikasi rate limit aktif setelah 5x gagal

### 1.5 JWT Service

- [x] Buat `JwtTokenService` dengan method:
  - `generateAccessToken(payload): string`
  - `generateRefreshToken(payload): string`
  - `verifyAccessToken(token): payload`
  - `verifyRefreshToken(token): payload`
- [x] Gunakan secret berbeda untuk access & refresh token (`JWT_ACCESS_SECRET`, `JWT_REFRESH_SECRET`)
- [x] Buat unit test untuk generate & verify token

---

## Definition of Done

- [x] `POST /api/v1/auth/register` berhasil membuat user baru dan return data user
- [x] `POST /api/v1/auth/login` berhasil return access & refresh token
- [x] Password tersimpan sebagai hash BCrypt di database
- [x] Email duplikat ditolak dengan HTTP 409
- [x] Validasi input bekerja (email format, password strength)
- [x] Rate limit login aktif (5x gagal → 429)
- [x] Access token berisi payload yang benar (`userId`, `role`)
- [x] Swagger docs lengkap untuk kedua endpoint
- [x] Unit test: AuthService (register, login, rate limit)
